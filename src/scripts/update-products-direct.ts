...
async function updateProductsDirect() {
  try {
    console.log('Starting direct product image URL update...');
    
    // Try to connect with a more robust connection string
    ...
    // If MongoDB connection fails, we can still provide the mapping for manual update
    console.log('\n📋 Manual Update Instructions:');
    console.log('If MongoDB connection continues to fail, you can manually update the products:');
    console.log('\nImage URL Mappings:');
    Object.entries(imageUrlMapping).forEach(([oldPath, newUrl]) => {
